            ...
            <t-content>
                ...
                <t-card :bordered="false" class="main_content_one">
                    <div class="search-list mb-3 ">
                        ...
                        <div >
                            <t-input v-model="panelData.formData.word" placeholder="词根" clearable></t-input>
                        </div>
                    ...
                    <div>
                        <t-table row-key="row_id" :data="panelData.detail" headerAffixedTop verticalAlign="bottom"
                            :style="{ width: `${screenWidth}px` }" :loading="loading" tableLayout="fixed" v-model:displayColumns="displayColumns"
                            ...
                            <template #campaign_cnt="{ row }">
                                <span class="hand_detail" @click="onClick(row.row_id, 'cnt')">{{
                                    exchangeRate(row.campaign_cnt, row.currency_code)
                                    }}</span>
                            </template>
                            <template #cost="{ row }">
...
import 'animate.css';
import { getAdvstrategydetail, getListCostData, getListCampaignData, addnegkeywords } from '@/api/intelligentDelivery';
...
// 超管进来没有操作栏
const displayColumns = ref<TableProps['displayColumns']>(accountData.role === 'employ'?
  ['serial-number', 'good_name','good_info.name','category_name','word','campaign_cnt','campaign_name_list','search_term_cnt',
    'search_term_list','statistic_days', 'impressions','clicks','cost', 'sale_amt','sale_num', 'order_num','cpc','acos', 'cvr',
    'cate_cvr', 'cate_cpc','cate_acos','adv_manager_name','adv_department_list_name','applyState','handle'
  ]: ['serial-number', 'good_name','good_info.name','category_name','word','campaign_cnt','campaign_name_list','search_term_cnt',
    'search_term_list','statistic_days', 'impressions','clicks','cost', 'sale_amt','sale_num', 'order_num','cpc','acos', 'cvr',
    'cate_cvr', 'cate_cpc','cate_acos','adv_manager_name','adv_department_list_name','applyState' 
  ]
);
const columnsDetail = reactive([
    ...
    { colKey: 'word', title: '词根',align: 'center', sortType: 'all', sorter: true },
    { colKey: 'campaign_cnt', title: '广告活动个数',align: 'center', width: 140, sortType: 'all', sorter: true },
    { colKey: 'campaign_name_list', title: '广告活动名称list',align: 'center', width: 160, sortType: 'all', sorter: true },
    { colKey: 'search_term_cnt', title: '搜索词个数',align: 'center', width: 160, sortType: 'all', sorter: true },
    { colKey: 'search_term_list', title: '搜索词list', align: 'center',width: 360, ellipsis: true, sortType: 'all', sorter: true },
    { colKey: 'statistic_days', title: '统计天数', align: 'center',width: 120, sortType: 'all', sorter: true },
    { colKey: 'impressions', title: '曝光量',align: 'center', sortType: 'all', sorter: true },
...
async function getCostChartData(row_id: string) {
    ...
    console.log(panelData.value.costData)
}
...
async function addWordApply(row: any) {
    ...
    const res = await addnegkeywords(form);
    console.log(res);
