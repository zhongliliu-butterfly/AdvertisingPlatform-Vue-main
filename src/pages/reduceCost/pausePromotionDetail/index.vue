                ...
                <t-card :bordered="false" class="main_content_one">
                    <div class="search-list mb-3 ">
                        ...
                        <div v-if="accountData.role==='root'">
                            ...
                            :options="panelData.managers_info" clearable filterable
                                    style="width: 360px;" :keys="{ label: 'name', value: 'name', children: 'child' }"
                                     />
                        </div>
                        ...
                        <div >
                            <t-input v-model="panelData.formData.department_name" placeholder="广告部门列表" clearable></t-input>
                        </div>
                        <div >
                        ...
                        <div >
                            <t-input v-model="panelData.formData.ad_group_name" placeholder="广告组名称" clearable></t-input>
                        </div>
                        ...
                        <t-table row-key="row_id" :data="panelData.detail" headerAffixedTop verticalAlign="bottom"
                            ...
                            <template #good_name="{ row }">
                                <div class="table-good">
                                    ...
                                    <div>
                                        <p>{{ row.good_info.asin }} <t-icon
                                                @click="WebApp.copyASIN(row.good_info.asin)" class="file_copy"
                                                name="file-copy" />
                                        </p>
                            ...
                            <template #cost="{ row }">
                                <span class="hand_detail" @click="onClick(row.row_id)">{{
                            ...
                            <template #handle="{ row }" >
                                <t-button v-if="row.status === false"  class="blue_btn"  @click="addWordApply(row)" :disabled="accountData.role !== 'employ'"  >应用</t-button>
...
import 'animate.css';
import { getAdvstrategydetail, getListCostData, pauseproduct, addproduct } from '@/api/intelligentDelivery';
import { getAllUser } from '@/api/home';
import * as WebApp from '@/utils/webapp';
import {  exchangeRate } from '@/utils/countryFile';
import { TabsProps, TableProps, DialogProps, MessagePlugin, TableRowData } from 'tdesign-vue-next';
...
const checkedValue=WebApp.getQueryFromUrl('menu_name')==='pausePromotion'?['3', '3-1']:['3', '3-2']
...
const columnsDetail = reactive([
    ...
    { colKey: 'campaign_name', title: '广告活动名称', width: 140 },
    { colKey: 'ad_group_id', title: '广告组ID', width: 140 },
    { colKey: 'ad_group_name', title: '广告组名称', width: 140 },
    { colKey: 'statistic_days', title: '统计天数', width: 140, sortType: 'all', sorter: true },
    { colKey: 'clicks', title: '点击量', sortType: 'all', sorter: true },
    {
        colKey: 'cost', title: '广告花费', width: 140, sortType: 'all', sorter: true
    },
    {
        colKey: 'sale_amt', title: '销售额', sortType: 'all', sorter: true,
    ...
    { colKey: 'sale_num', title: '销售量', sortType: 'all', sorter: true },
    { colKey: 'order_num', title: '订单量', sortType: 'all', sorter: true },
    { colKey: 'cpc', title: '竞价', sortType: 'all', sorter: true ,
    cell: (h: any, params: { row: any }) => {
            const { row } = params;
    ...
    {
        colKey: 'acos', title: 'ACOS', sortType: 'all', sorter: true,
        cell: (h: any, params: { row: any }) => {
            const { row } = params;
            if (row.sale_amt === 0) {
                return '/';
    ...
    {
        colKey: 'cvr', title: '转化率', sortType: 'all', sorter: true,
        cell: (h: any, params: { row: any }) => {
            const { row } = params;
    ...
    {
        colKey: 'cate_cvr', title: '类目CVR', width: 140, sortType: 'all', sorter: true,
        cell: (h: any, params: { row: any }) => {
            const { row } = params;
    ...
    {
        colKey: 'cate_cpc', title: '类目CPC', width: 140, sortType: 'all', sorter: true,
        cell: (h: any, params: { row: any }) => {
            const { row } = params;
    ...
    {
        colKey: 'cate_acos', title: '类目ACOS', width: 140, sortType: 'all', sorter: true,
        cell: (h: any, params: { row: any }) => {
            const { row } = params;
            return (row.cate_acos*100).toFixed(2) + '%';
        },
    },
    { colKey: 'adv_manager_name', title: '广告负责人', width: 120 },
    { colKey: 'adv_department_list_name', title: '广告部门列表', width: 180 },
    { colKey: 'applyState', title: '应用状态' },
...
async function addWordApply(row: any) {
    ...
    const res = strategyList.action_name === '暂停推广品' ? await pauseproduct(form) : await addproduct(form);
    console.log(res)
